  ...
  return (
    <header className="text-white body-font fixed inset-x-0 z-50 bg-darkblue">
      <div className="mx-auto py-2 px-5 flex flex-row justify-between items-center lg:py-4">
        ...
        >
          <span className="text-xl cursor-pointer hover:text-dark-orange font-medium">Home</span>
        </Link>
        <nav
          data-aos="fade-left"
          data-aos-duration="1000"
          ...
          {links.map((item) => {
            return (
              <Link
                ...
                duration={750}
                to={item.link}
                className="mr-5 cursor-pointer hover:text-dark-orange md:mr-12 font-medium"
              >
                {item.title}
              </Link>
            );
